...
function isValidDate(s) { return typeof s === "string" && /^\d{4}-\d{2}-\d{2}$/.test(s) && !Number.isNaN(new Date(s + "T00:00:00").getTime()); }
...
function parseRules(text, now = new Date()) {
...
function derivePet(tasks, pet, meta, now = new Date()) {
...
module.exports = { pad2, dateToStr, todayStr, monthKeyOf, shiftDays, mondayOf, isValidDate, resolveDate, CATEGORIES, categoryMeta, normalizeCategory, PRIORITIES, normalizePriority, parseRules, xpFor, levelInfo, STAGE_META, daysBetween, streakOf, derivePet, computeStats, fallbackSummary, uid };
